"""
etl/loaders/load_indicators.py — v4

CORREÇÕES v4:
- ROWID auto-increment — nunca mais conflito de ID
- DELETE simples e seguro (sem FK constraints)
- ensure_dims usa queries DuckDB nativas
...
def load_processed_file(conn, path: Path) -> int:
    ...
    cols = [
        "country_code", "city_id", "indicator_code", "scenario_code",
...
def run():
    ...
    with get_connection() as conn:
        ...
        total_loaded = 0
        for path in files:
            total_loaded += load_processed_file(conn, path)
            print()
