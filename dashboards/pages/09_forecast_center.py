    ...
    if result is None:
        ...
        st.markdown(
            ...
            - **Modelo**: bootstrap paramétrico — variações anuais históricas
              modeladas como distribuição normal
              (μ = {result['mu']*100:.2f}%, σ = {result['sigma']*100:.2f}% a.a.)
            - **Última observação**: {result['last_observed_value']:,.2f}
              ({result['last_observed_year']})
            - **Simulações**: 20.000 trajetórias · resultado em cache por 1 hora
            - **Limitações**: assume normalidade dos incrementos (ignora fat
              tails e regimes), ignora correlação entre indicadores e países,
              não modela choques exógenos (Copa 2026, geopolítica).
              EVT e modelos estruturais com shocks exógenos ficam para a
              fase pós-MVP.
            """
        )
else:
